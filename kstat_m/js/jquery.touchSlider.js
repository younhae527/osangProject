...
// 플리킹 탭 메뉴 부분 추가
function view_tab(el){
	if(el == 1){
		if($('.tab_type3').find('.on').prev().prop("tagName") == 'A'){
			$('.tab_type3').find('.on').removeClass('on').prev().addClass('on');
		}
	}else if(el == -1){
		if($('.tab_type3').find('.on').next().prop("tagName") == 'A'){
			$('.tab_type3').find('.on').removeClass('on').next().addClass('on');
		}
	}
}
// 플리킹 탭 메뉴 부분 추가
...
(function ($) {
	...
	var touchSlider = {
		...
		init : function () {
			...
			if(this.opts.btn_prev && this.opts.btn_next) {
				...
				this.opts.btn_next.unbind("click").bind("click", function() {
					if($('.subConWrap #s_menu').length != 0 && parseInt($('.subConWrap #s_menu li:last-child').prev().prev().css('left')) <= 0){
		...
		touchmove : function (e) {
			...
			} else if($('.subConWrap #s_menu').length == 0 || parseInt($('.subConWrap #s_menu li:last-child').prev().prev().css('left')) >= 0){
				if((e.type == "touchmove" && e.originalEvent.touches.length <= 1) || e.type == "drag") {
					...
					for(var i=0; i<this._len; ++i) {
						...
						if(this.opts.supportsCssTransitions && this.opts.transition) {
							...
							this._list.eq(i).css("left", tmp + "px");this._list.eq(i).removeClass('ddddddddddddddddddddd');
						}
				...
				this._list.each(function (i, el) {
					...
					if(_this.opts.supportsCssTransitions && _this.opts.transition) {
						...
						$(this).animate({"left": _this._pos[i] + "px"}, speed, function(){
							if($('.subConWrap').find('#view_tab').length != 0){
								$('.tab_type3').removeClass('fixed');
								view_tab_f();
								view_tab_h();
								var v_top =$(document).scrollTop();
								//v_top -= 100;
								$(document).scrollTop(v_top);
							}
							if($('.subConWrap #s_menu').length != 0){
